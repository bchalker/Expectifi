...
function DirectUsCityItem({
  ...
  const distanceMiles = getDirectUsCityDistanceMiles(
    city,
    destinationAirportCode,
  );
  ...
  return (
    <li className="wtr-getting-there__us-city">
      <span className="wtr-getting-there__us-city-label">
        {label}
        {displayIata ? (
          <span className="wtr-getting-there__us-city-iata tabular-nums">
            {" "}
            ({displayIata})
          </span>
        ) : null}
    ...
    >
      <div className="wtr-getting-there__airport-body">
        ...
        {showPrimaryBadge && isPrimary ? (
          <p className="wtr-getting-there__airport-primary-helper font-xs">
            Primary
          </p>
        ) : null}
      </div>
      <footer className="wtr-getting-there__airport-footer">
        ...
        <span className="wtr-getting-there__airport-code tabular-nums">
          {airport.code}
        </span>
      </footer>
    </li>
...
export function DestinationGettingThereTab({
  country,
  staggerClassName,
  staggerStyle,
}: Props) {
  ...
  const countableAirports = data.main_airports.filter(
    (airport) =>
      airport.code?.trim() && airport.code.toLowerCase() !== "various",
  );
  ...
  return (
    <div className="wtr-getting-there">
      <section
        ...
        {...staggerSectionProps(
          0,
          "detail-panel-card wtr-getting-there__group",
          staggerClassName,
          staggerStyle,
        )}
      >
      ...
      >
        ...
        <p className="wtr-getting-there__airlines-list">
          {formatAirlinesList(data.airlines)}
        </p>
      </section>
      ...
      <aside
        ...
        {...staggerSectionProps(
          3,
          "wtr-getting-there__tip",
          staggerClassName,
          staggerStyle,
        )}
      >
        <IconBulb
          className="wtr-getting-there__tip-icon"
          size={18}
          stroke={1.5}
          aria-hidden
        />
        <div className="wtr-getting-there__tip-copy">
          ...
          <p className="wtr-getting-there__tip-value">
            {data.best_booking_tip}
          </p>
        </div>
      </aside>
      ...
      <a
        ...
        {...staggerSectionProps(
          5,
          "wtr-getting-there__flights-link",
          staggerClassName,
          staggerStyle,
        )}
      >
