...
function WeatherSkeleton({
  ...
  return (
    <article
      className="wtr-weather-tab wtr-weather-tab--loading"
      aria-busy="true"
    ...
    >
      <div
        {...staggerSectionProps(
          0,
          "wtr-weather-tab__skeleton-title",
          staggerClassName,
          staggerStyle,
        )}
      />
      <div
        {...staggerSectionProps(
          1,
          "wtr-weather-tab__skeleton-summary",
          staggerClassName,
          staggerStyle,
        )}
      />
      <div
        {...staggerSectionProps(
          2,
          "wtr-weather-tab__skeleton-metrics",
          staggerClassName,
          staggerStyle,
        )}
      >
      ...
      <div
        {...staggerSectionProps(
          3,
          "wtr-weather-tab__skeleton-chart",
          staggerClassName,
          staggerStyle,
        )}
      />
...
function MetricColumn({
  ...
  return (
    <div className="wtr-weather-tab__metric">
      <p
        className={[
          "wtr-weather-tab__metric-primary tabular-nums",
          primaryClassName,
        ]
          .filter(Boolean)
          .join(" ")}
      ...
      {secondary ? (
        <p
          className={[
            "wtr-weather-tab__metric-secondary tabular-nums",
            secondaryClassName,
          ]
            .filter(Boolean)
            .join(" ")}
...
export function WeatherTab({
  ...
  const summaryCardClassName = [
    "detail-panel-card wtr-weather-tab__summary-card",
    ...
    <div className="wtr-city-detail__tab-content wtr-city-detail__tab-content--weather">
      <article className="wtr-weather-tab" aria-label="Climate and weather">
        ...
        >
          ...
          <div
            className="wtr-weather-tab__unit-toggle"
            role="group"
        ...
        >
          <div className="wtr-weather-tab__summary-top">
            {showFitScore && climateNotes ? (
              <div
                className="wtr-weather-tab__fit-score"
                aria-label={`Climate fit score ${climateNotes.climateScore} out of 100`}
              >
                <span className="wtr-weather-tab__fit-score-value tabular-nums">
                  {climateNotes.climateScore}
                </span>
              </div>
            ) : null}
          ...
          <div className="wtr-weather-tab__summary-retiree">
            ...
            <p>
              {retireeNote.label ? (
                <strong className="wtr-weather-tab__retiree-label">
                  {retireeNote.label}
                </strong>
              ) : null}
        ...
        >
          <div className="wtr-weather-tab__metrics">
            <MetricColumn
              label="Avg high"
              primary={avgHigh.primary}
              secondary={avgHigh.secondary}
            />
            <MetricColumn
              label="Avg low"
              primary={avgLow.primary}
              secondary={avgLow.secondary}
            />
            <MetricColumn
              label="Avg humidity"
              primary={
        ...
        <section
          ...
          {...staggerSectionProps(
            staggerIdx++,
            "detail-panel-card wtr-weather-tab__chart-card",
            staggerClassName,
            staggerStyle,
          )}
        >
          <div className="wtr-weather-tab__section-header">
            <h3
              className="wtr-weather-tab__section-title"
              id="wtr-weather-chart-heading"
            >
            ...
            <p className="wtr-weather-tab__section-helper">
              Avg highs{" "}
              {formatTemp(
                Math.max(...climate.monthly.map((m) => m.avgHighC)),
                tempUnit,
              )}{" "}
              / lows{" "}
              {formatTemp(
