import { IconEdit, IconTrendingDown, IconTrendingUp } from '@tabler/icons-react'
...
export type GrowthSliderLabelProps = {
  ...
  /** Range row rendered after the headline; custom-return suffix renders below the slider. */
  slider: ReactNode
}
...
export function GrowthSliderLabel({
  ...
  const customRet = positions.filter((p) => positionUsesCustomReturnMode(p, blendedRate))
  const customBrk = brkList.filter((p) => positionUsesCustomReturnMode(p, brkBlend))
  ...
  const allRetCustom =
    positions.length === 0 || positions.every((p) => positionUsesCustomReturnMode(p, blendedRate))
  const allBrkCustom =
    brkList.length === 0 || brkList.every((p) => positionUsesCustomReturnMode(p, brkBlend))
  ...
  const [popStyle, setPopStyle] = useState<{ left: number; top: number }>({ left: 0, top: 0 })

  useLayoutEffect(() => {
    if (!anchor) return
    const half = 120
    const x = Math.min(window.innerWidth - half - 8, Math.max(half + 8, anchor.left + anchor.width / 2))
  ...
  useEffect(() => {
    ...
    function onPointerDown(e: MouseEvent) {
      ...
      if ((e.target as HTMLElement | null)?.closest?.('[data-growth-slider-ticker]')) return
      setHoverId(null)
      setAnchor(null)
    }
  ...
  const hovered = hoverId ? customPositions.find((p) => p.id === hoverId) : undefined
  ...
  function popoverInner(p: PositionReturnModel) {
    ...
    const showYearGrid = p.returnMode === 'peryear' || p.returnMode === 'scenario'
    ...
    return (
      <>
        ...
        <div className="growth-slider-hover-pop__scenario-row">
          <span className="growth-slider-hover-pop__scenario-kicker">Scenario</span>
          {scenarioChoice === 'default' ? (
            <span className="growth-slider-hover-pop__scenario-plain">{scenarioLabel}</span>
          ) : (
            <span className={`fidelity-agg-scenario-trigger ${scenarioBadgeClass(scenarioChoice)}`}>
              {scenarioLabel}
            </span>
          )}
        </div>
        {p.returnMode === 'flat' && scenarioChoice === 'custom' ? (
          <p className="growth-slider-hover-pop__mode-note">Applied all years</p>
        ) : null}
        {showYearGrid ? (
          <div className="growth-slider-hover-pop__rows">
            {years.map((y, i) => (
              <div key={`${p.id}-y-${y}`} className="growth-slider-hover-pop__row">
                <span>{y}</span>
  ...
  return (
    <div className="growth-slider-label">
      <div className="growth-slider-label__headline-stack">
        <span className="growth-slider-label__pct growth-slider-label__pct--hero strip-equation-main-val--tween">
          {headlinePct}%
        </span>
