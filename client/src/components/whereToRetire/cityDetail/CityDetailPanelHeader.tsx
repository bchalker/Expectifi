...
export const CityDetailPanelHeader = memo(function CityDetailPanelHeader({
  ...
  const fitScore = Math.max(
    0,
    Math.min(100, Math.round(headerScore.displayScore)),
  );
  const { band: scoreBand, label: scoreBandLabel } =
    scoreDetailBandFromScore(fitScore);
  const fitScoreColors = getFitScoreColors(fitScore);
  const showCaution = hasReconsiderTravelAdvisory(country);

  return (
    <header
      className="wtr-city-detail__header"
        ...
        <div className="wtr-city-detail__identity">
          ...
          <div className="wtr-city-detail__country-line">
            <CountryFlag
              country={country}
              size="s"
              className="wtr-city-detail__flag"
            />
            <span className="wtr-city-detail__country-text">
              ...
              {showCaution ? (
                <>
                  <span
                    className="wtr-city-detail__country-divider"
                    aria-hidden
                  >
                    ·
                  </span>
              ...
              >
                <span
                  className={[
                    ...
                    countryExcluded &&
                      "wtr-city-detail__country-forbid--active",
                  ]
                    .filter(Boolean)
                    .join(" ")}
                  aria-label={
        ...
        <div className="wtr-city-detail__cost-block">
          ...
          {monthlySurplus > 0 ? (
            <span className="wtr-city-detail__surplus tabular-nums">
              <IconPlus size={12} stroke={1.5} aria-hidden />
          ...
          >
            <div className="wtr-city-detail__fit-score-value-row">
              <p className="wtr-city-detail__fit-score-value tabular-nums">
                <span className="wtr-city-detail__fit-score-number">
                  {fitScore}
                </span>
              </p>
