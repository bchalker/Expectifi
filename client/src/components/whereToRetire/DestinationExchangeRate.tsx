...
export function DestinationExchangeRate({
  ...
  const rootClass = [
    "detail-panel-card wtr-exchange-rate",
  ...
  if (isUsd) {
    if (variant === "compact") {
      return (
        <p
          className={rootClass}
          {...staggerProps(staggerClassName, staggerStyle, 0)}
        >
          Budget in USD
        </p>
      );
    ...
    return (
      ...
      >
        <p className="wtr-exchange-rate__copy">
          Prices and budget estimates are in US dollars. This destination uses
          the US dollar.
        </p>
        {planMonthlyIncome > 0 ? (
  ...
  const localMonthlyIncome =
    planMonthlyIncome > 0 ? planMonthlyIncome * currency.rate : null;
  const incomeLabel =
    ...
    return (
      ...
      >
        <IconArrowsExchange
          ...
          stroke={1.5}
          aria-hidden
          className="wtr-exchange-rate__compact-icon"
        />
  ...
  return (
    <section
      className={rootClass}
      aria-label="Exchange rate vs US dollar"
      {...staggerProps(staggerClassName, staggerStyle, 0)}
    >
      <div className="wtr-exchange-rate__top">
        <div className="wtr-exchange-rate__rates">
          <p className="wtr-exchange-rate__rate-row">
            ...
            >
              <span className="wtr-exchange-rate__spot-rate tabular-nums">
                {rateLabel}
              </span>
            </Tooltip>
          </p>
        ...
        {purchasingMeta ? (
          <Tooltip
            ...
            content={
              <>
                Living-cost comparison for {city.city} vs a typical US city in
                our data.
                <br />
                The exchange rate above is the same for all countries using{" "}
