...
function HsaAnalysisBlock({ hsaResult, hsaBalance, hsaSavings }: HsaAnalysisBlockProps) {
    ...
    return (
      <div className="life-events-hsa life-events-hsa--teal">
        <div className="life-events-hsa__header">
          ...
          <span className="life-events-hsa__pill life-events-hsa__pill--teal">No portfolio impact</span>
        </div>
        <p className="life-events-hsa__body">
          Your HSA balance of {formatCurrency(hsaBalance)} fully covers this expense. Your retirement
          portfolio is untouched. Your HSA is doing exactly what it was designed for.
        </p>
      </div>
    )
  }
    ...
    return (
      <div className="life-events-hsa life-events-hsa--teal">
        ...
        <p className="life-events-hsa__savings">
          Your HSA saves you {formatCurrency(hsaSavings)} in lost compounding compared to paying this
          from your retirement portfolio.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="life-events-hsa life-events-hsa--amber">
      ...
      <p className="life-events-hsa__body">
        The full {formatCurrency(grossExpense)} comes from your retirement portfolio. An HSA is one of
        the most tax-efficient tools for medical costs in retirement. Contributions go in pre-tax, grow
        tax-free, and withdraw tax-free for qualified medical expenses. Even a modest HSA balance reduces
        what retirement has to cover.
      </p>
    </div>
  )
}
...
export default function GrowthEventCard({
  ...
  const yearMax = Math.max(currentYear, retirementYear - 1)
  ...
  const durationMax = config.durationMax ?? 20

  const mortgageExtras = isMortgageEventExtras(config.extras) ? config.extras : null
  const showHsaAnalysis = isMedicalEventExtras(config.extras)

  const [mortgageRate, setMortgageRate] = useState(
    mortgageExtras?.mortgageRateDefault ?? 0.04,
  )
  const [monthlyPayment, setMonthlyPayment] = useState(
    mortgageExtras?.monthlyPaymentDefault ?? 1500,
  )
  ...
  const { futureValue, totalOutflow, rating: calcRating } = calcEventValues(
    calcAmount,
    yearValue,
    retirementYear,
    retirementPortfolio,
  ...
  const displayCurrent = retirementPortfolio - otherActiveImpact
  const displayAfter = displayCurrent - futureValue
  const displayDelta = displayAfter - displayCurrent
  ...
  const mortgageRateRangeId = `life-events-${state.id}-mortgage-rate-range`
  const monthlyPaymentValueId = `life-events-${state.id}-monthly-payment-value`
  const monthlyPaymentRangeId = `life-events-${state.id}-monthly-payment-range`

  const showTradeoff = mortgageExtras?.showTradeoffAnalysis === true
  const tradeoff = showTradeoff
    ? calcMortgageTradeoff(state.amount, mortgageRate, growthRate, yearValue, retirementYear)
    : null
  const yearsRemaining = Math.max(0, retirementYear - yearValue)

  const showHsaSubtext =
    hsaResult != null && hsaResult.hsaOffset > 0 && !hsaResult.fullyCovered
          ...
          <>
            <div className="life-events-event__impact-row">
              ...
              <span className="life-events-event__impact-group">
                ...
                {futureValue > 0 ? (
                  <span className="life-events-event__impact-delta">
                    ({formatCurrencyCompact(displayDelta)})
                  </span>
                ) : null}
              </span>
            </div>
              ...
              <div className="life-events-slider-group">
                <div className="life-events-field">
                  <div className="life-events-field__row">
                    <label className="life-events-field__label" htmlFor={amountValueId}>
                      {config.amountLabel}
                    </label>
                    <LifeEventFieldValue
                      id={amountValueId}
                      ...
                      onCommit={(amount) => onStateChange(state.id, { amount })}
                    />
                  </div>
                  <input
                    id={amountRangeId}
                    ...
                    value={state.amount}
                    onChange={(e) => onStateChange(state.id, { amount: Number(e.target.value) })}
                ...
                <div className="life-events-field">
                  <div className="life-events-field__row">
                    <label className="life-events-field__label" htmlFor={yearValueId}>
                      {config.yearLabel}
                    </label>
                    <LifeEventFieldValue
                      id={yearValueId}
                      ...
                      parseDisplay={(raw) => parseInt(raw.replace(/,/g, ''), 10) || 0}
                  ...
                  <input
                    ...
                    step={1}
                    value={yearValue}
                    onChange={(e) => onStateChange(state.id, { year: Number(e.target.value) })}
                ...
                {config.isRecurring ? (
                  <div className="life-events-field">
                    <div className="life-events-field__row">
                      <label className="life-events-field__label" htmlFor={durationValueId}>
                      ...
                      <LifeEventFieldValue
                        ...
                        parseDisplay={(raw) => parseInt(raw.replace(/,/g, ''), 10) || 0}
                        ariaLabel={config.durationLabel ?? 'For how many years?'}
                        onCommit={(duration) => onStateChange(state.id, { duration })}
                      />
                    </div>
                    <input
                      id={durationRangeId}
                      ...
                      value={durationValue}
                      onChange={(e) =>
                        onStateChange(state.id, { duration: Number(e.target.value) })
                      }
                      aria-label={config.durationLabel ?? 'For how many years?'}
                    />
                  </div>
                ) : null}
                ...
                {showTradeoff && mortgageExtras ? (
                  <>
                    <div className="life-events-field">
                      <div className="life-events-field__row">
                        <label className="life-events-field__label" htmlFor={mortgageRateValueId}>
                          Your mortgage rate
                        </label>
                        <LifeEventFieldValue
                          ...
                          suffix="%"
                          formatDisplay={(rate) => rate.toFixed(2)}
                          round={(rate) => Math.round(rate * 100) / 100}
                          ariaLabel="Your mortgage rate"
                          onCommit={(ratePct) => setMortgageRate(ratePct / 100)}
                        />
                      </div>
                      <input
                        id={mortgageRateRangeId}
                        className="life-events-field__range"
                        type="range"
                        min={mortgageExtras.mortgageRateMin}
                        max={mortgageExtras.mortgageRateMax}
                        step={mortgageExtras.mortgageRateStep}
                        value={mortgageRate * 100}
                        onChange={(e) => setMortgageRate(Number(e.target.value) / 100)}
                        aria-label="Your mortgage rate"
                      />
                    </div>

                    <div className="life-events-field">
                      <div className="life-events-field__row">
                        <label className="life-events-field__label" htmlFor={monthlyPaymentValueId}>
                        ...
                        <LifeEventFieldValue
                          ...
                          prefix="$"
                          formatDisplay={fmtInput}
                          ariaLabel="Current monthly payment"
                          onCommit={setMonthlyPayment}
                        />
                      </div>
                      <input
                  ...
                  <div className="life-events-tradeoff__grid">
                        <div
                          className={[
                            ...
                            !tradeoff.investingWins && 'life-events-tradeoff__card--recommended',
                          ]
                            .filter(Boolean)
                            .join(' ')}
                        >
                          <div className="life-events-tradeoff__card-head">
                            <span className="life-events-tradeoff__card-title">
                              ...
                              <span className="life-events-tradeoff__card-label">Pay off early</span>
                            </span>
                            {!tradeoff.investingWins ? <TradeoffRecommendedPill /> : null}
                          </div>
                          <div className="life-events-tradeoff__row">
                            <span className="life-events-tradeoff__row-label">Portfolio cost</span>
                          ...
                          <div className="life-events-tradeoff__row">
                            <span className="life-events-tradeoff__row-label">Monthly freed</span>
                            <span className="life-events-tradeoff__row-value life-events-tradeoff__row-value--positive">
                              +${monthlyPayment.toLocaleString()}/mo
                            </span>
                          </div>
                          <div className="life-events-tradeoff__row">
                            <span className="life-events-tradeoff__row-label">Interest saved</span>
                        ...
                        >
                          <div className="life-events-tradeoff__card-head">
                            <span className="life-events-tradeoff__card-title">
                              ...
                              <span className="life-events-tradeoff__card-label">Stay invested</span>
                            </span>
                            {tradeoff.investingWins ? <TradeoffRecommendedPill /> : null}
                          </div>
                          <div className="life-events-tradeoff__row">
                            <span className="life-events-tradeoff__row-label">Portfolio gain</span>
                            <span className="life-events-tradeoff__row-value life-events-tradeoff__row-value--positive">
                              {formatCurrency(tradeoff.investmentGain)}
                            </span>
                          </div>
                          <div className="life-events-tradeoff__row">
                            <span className="life-events-tradeoff__row-label">Mortgage continues</span>
                            <span className="life-events-tradeoff__row-value life-events-tradeoff__row-value--negative">
                              -${monthlyPayment.toLocaleString()}/mo
                            </span>
                          </div>
                          <div className="life-events-tradeoff__row">
                            <span className="life-events-tradeoff__row-label">Net advantage</span>
                            <span className="life-events-tradeoff__row-value">
                              {formatCurrency(Math.abs(tradeoff.netAdvantageOfInvesting))}
                            </span>
                          </div>
                        </div>
                      </div>
