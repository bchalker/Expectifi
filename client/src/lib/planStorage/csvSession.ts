import type { StoredFidelityImportV2 } from '../fidelityStorage'
 ...
 */
export function saveCsvSession(payload: StoredFidelityImportV2): void {
...
export function loadCsvSession(): StoredFidelityImportV2 | null {
  if (typeof window === 'undefined') return null
  try {
    const raw = sessionStorage.getItem(CSV_SESSION_KEY)
    if (!raw) return null
    const parsed = JSON.parse(raw) as StoredFidelityImportV2
