...
export function filterMonthsByQuery(query: string): typeof DOB_MONTHS {
  const q = query.trim().toLowerCase()
  if (!q) return [...DOB_MONTHS]
  const numeric = /^\d{1,2}$/.test(q) ? q.padStart(2, '0') : null
  return DOB_MONTHS.filter((m) => {
    if (numeric && m.id === numeric) return true
    return (
      m.label.toLowerCase().startsWith(q) ||
      m.short.toLowerCase().startsWith(q)
    )
  })
}

export function monthIdFromQuery(query: string): string | null {
  const q = query.trim().toLowerCase()
  if (!q) return null
  const matches = filterMonthsByQuery(q)
  const exact = matches.find(
    (m) =>
      m.label.toLowerCase() === q ||
      m.short.toLowerCase() === q ||
      m.id === q.padStart(2, '0'),
  )
...
export function filterYearsByQuery(years: number[], query: string): number[] {
  const q = query.trim().replace(/[^\d]/g, '')
  if (!q) return years
  return years.filter((y) => String(y).startsWith(q))
}

export function yearFromQuery(query: string, years: number[]): number | null {
  const q = query.trim().replace(/[^\d]/g, '')
  if (!q) return null
  const parsed = Number.parseInt(q, 10)
  if (!Number.isFinite(parsed)) return null
  if (years.includes(parsed) && q.length === 4) return parsed
  const prefixMatches = filterYearsByQuery(years, q)
  if (prefixMatches.length === 1) return prefixMatches[0]
  const exact = years.find((y) => String(y) === q)
  return exact ?? null
}

export function nearestValidYear(parsed: number, years: number[]): number | null {
