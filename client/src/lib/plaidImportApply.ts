...
function stripCsvImportBatches(stored: StoredFidelityImportV2 | null): StoredFidelityImportV2 | null {
  ...
  const batches = stored.batches.filter((b) => b.plaidItemId)
...
export function applyPlaidHoldingsSnapshot(snapshot: PlaidHoldingsSnapshot) {
  ...
  const existing = loadStoredFidelityImport()
  const withoutCsv = stripCsvImportBatches(existing)
  const withoutItem = stripPlaidBatches(withoutCsv, [snapshot.itemId])
...
export function applyPlaidHoldingsSnapshots(snapshots: PlaidHoldingsSnapshot[]) {
  ...
  const existing = loadStoredFidelityImport()
  const withoutCsv = stripCsvImportBatches(existing)
  const withoutItems = stripPlaidBatches(withoutCsv, itemIds)
